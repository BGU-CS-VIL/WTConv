...
// =============================================================================
// Haar 2D Wavelet Transform - Single Level Forward & Backward
...
// -----------------------------------------------------------------------------
template<typename T>
__global__ void haar2d_forward_kernel(
    ...
    if (y < H2 && x < W2) {
        ...
        // Load as native type, convert to float for computation
        float a = to_float(input[in_offset + y0 * W + x0]);
        float b_val = to_float(input[in_offset + y0 * W + x1]);
        float c = to_float(input[in_offset + y1 * W + x0]);
        float d = to_float(input[in_offset + y1 * W + x1]);
        
        float sum_ac = a + c, sum_bd = b_val + d;
        float diff_ac = a - c, diff_bd = b_val - d;
        
        int out_idx = y * W2 + x;
        int plane = H2 * W2;
        int out_offset = bc * 4 * plane;
        
        // Convert back to native type for storage
        output[out_offset + 0 * plane + out_idx] = from_float<T>(0.5f * (sum_ac + sum_bd));   // LL
        output[out_offset + 1 * plane + out_idx] = from_float<T>(0.5f * (diff_ac + diff_bd)); // LH
        output[out_offset + 2 * plane + out_idx] = from_float<T>(0.5f * (sum_ac - sum_bd));   // HL
        output[out_offset + 3 * plane + out_idx] = from_float<T>(0.5f * (diff_ac - diff_bd)); // HH
    }
}
...
// -----------------------------------------------------------------------------
template<typename T>
__global__ void haar2d_backward_kernel(
    const T* __restrict__ grad_output,  // (B*C, 4, H2, W2)
    ...
    if (y < H2 && x < W2) {
        ...
        float g_ll = to_float(grad_output[grad_offset + 0 * plane + idx]);
        float g_lh = to_float(grad_output[grad_offset + 1 * plane + idx]);
        float g_hl = to_float(grad_output[grad_offset + 2 * plane + idx]);
        float g_hh = to_float(grad_output[grad_offset + 3 * plane + idx]);
        ...
        grad_input[in_offset + y0 * W + x0] = from_float<T>(grad_a);
        if (x1 < W) grad_input[in_offset + y0 * W + x1] = from_float<T>(grad_b);
        if (y1 < H) grad_input[in_offset + y1 * W + x0] = from_float<T>(grad_c);
        if (x1 < W && y1 < H) grad_input[in_offset + y1 * W + x1] = from_float<T>(grad_d);
    }
}
...
void haar2d_forward(torch::Tensor input, torch::Tensor output) {
    ...
    if (input.dtype() == torch::kFloat32) {
        haar2d_forward_kernel<float><<<grid, block>>>(
            input.data_ptr<float>(),
            output.data_ptr<float>(),
            C, H, W, H2, W2
        );
    } else if (input.dtype() == torch::kFloat16) {
        haar2d_forward_kernel<__half><<<grid, block>>>(
            reinterpret_cast<const __half*>(input.data_ptr<at::Half>()),
            reinterpret_cast<__half*>(output.data_ptr<at::Half>()),
            C, H, W, H2, W2
        );
    } else if (input.dtype() == torch::kBFloat16) {
        haar2d_forward_kernel<__nv_bfloat16><<<grid, block>>>(
...
void haar2d_backward(torch::Tensor grad_output, torch::Tensor grad_input) {
    TORCH_CHECK(grad_output.dim() == 5, "grad_output must be 5D (B, C, 4, H2, W2)");
    TORCH_CHECK(grad_output.is_cuda(), "grad_output must be on CUDA");
    
    int B = grad_output.size(0);
    int C = grad_output.size(1);
    int H2 = grad_output.size(3);
    int W2 = grad_output.size(4);
    ...
    if (grad_output.dtype() == torch::kFloat32) {
        haar2d_backward_kernel<float><<<grid, block>>>(
            grad_output.data_ptr<float>(),
            grad_input.data_ptr<float>(),
            H, W, H2, W2
        );
    } else if (grad_output.dtype() == torch::kFloat16) {
        haar2d_backward_kernel<__half><<<grid, block>>>(
        ...
        haar2d_backward_kernel<__nv_bfloat16><<<grid, block>>>(
